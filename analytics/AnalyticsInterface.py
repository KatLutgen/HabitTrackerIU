...
class AnalyticsInterface:
    ...
    def show_streaks_consistency(self):
        try:
            ...
            cursor.execute("""
                SELECT Habit.HabitId, Habit.Name
                FROM Habit
                WHERE Habit.UserId = ?
            """, (self.current_user_id,))
            ...
            for habit_id, habit_name in habits:
                cursor.execute("""
                    SELECT HabitRecord.Success, DATE(HabitRecord.LogDate) AS LogDate
                    FROM HabitRecord
                    WHERE HabitRecord.HabitId = ?
                    ORDER BY HabitRecord.LogDate DESC
                """, (habit_id,))
                ...
                for success, log_date in records:
                    record_date = datetime.strptime(log_date, "%Y-%m-%d").date()
                ...
                habits_data.append((habit_name, streak))
